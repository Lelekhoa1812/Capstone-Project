...
export async function GET(req: NextRequest) {
  ...
  try {
    console.log("🔍 Fetching processed files from Firebase Storage...");
    ...
    const filenamePattern = /^skyledge\/processed\/(\d{3})_(\d{4}-\d{2}-\d{2})_processed\.csv$/;
    ...
    processedFiles.sort((a, b) => new Date(b.timeCreated).getTime() - new Date(a.timeCreated).getTime());

    console.log(`✅ Processed ${processedFiles.length} processed files`);
